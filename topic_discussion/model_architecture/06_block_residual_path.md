...
本页在 Task0 中负责“组装和复核”。建议在读完 Tokenization、Norm、Attention、RoPE 和 MLP 后进入这里；它不再重复介绍单个组件，而是检查这些组件如何共同形成可训练、可推理的 Block。
...
- [Part 02 · 01 RMSNorm](../../02_PyTorch_Algorithms/01_RMSNorm_Tutorial.ipynb)、[Part 02 · 02 SwiGLU](../../02_PyTorch_Algorithms/02_SwiGLU_Activation.ipynb)、[Part 02 · 03 RoPE](../../02_PyTorch_Algorithms/03_RoPE_Tutorial.ipynb) 和 [Part 02 · 04 Attention（MHA/GQA）](../../02_PyTorch_Algorithms/04_Attention_MHA_GQA.ipynb) 的组件都在这里重新组装
- [Part 02 · 05 LLaMA3 Block](../../02_PyTorch_Algorithms/05_LLaMA3_Block_Tutorial.ipynb) 是最直接的 block 级案例
- [Part 02 · 06 MoE Router](../../02_PyTorch_Algorithms/06_MoE_Router.ipynb) 和 [Part 02 · 07 MoE 负载均衡损失](../../02_PyTorch_Algorithms/07_MoE_Load_Balancing_Loss.ipynb) 展示把 MLP 扩展为 MoE 的方式
- [Part 02 · 08 架构技巧](../../02_PyTorch_Algorithms/08_Architecture_Tricks.ipynb) 和 [Part 02 · 61 模型架构探索](../../02_PyTorch_Algorithms/61_Model_Architecture_Exploration.ipynb) 用于核对真实模型中的 block 变体
...
进入 [08 代表模型与结构对照](./08_representative_models.md)，把这套 Block 结构放回真实模型中核对；如果还没有看 MLP，请先阅读 [07 MLP / FFN Evolution](./07_mlp_ffn_evolution.md)。
