...
| 观察到的变化 | 不能直接推出什么 | 下一步检查 |
|:---|:---|:---|
| win-rate 上升，safety 下降 | 不能直接 accept | 回看安全集、失败案例和最低安全阈值 |
| 格式遵循变好，任务正确率下降 | 不能把格式稳定当作能力提升 | 分开统计格式、正确性和拒答指标 |
| reward 上升，独立评测不变 | 不能把 reward 当作真实能力 | 检查 verifier / judge 是否过窄或被投机 |
| 离线指标变好，在线延迟或波动变差 | 不能直接进入生产 | 检查更新时延、policy freshness 和回滚条件 |
