...
Task5 和 Task6 的分界在于：Task5 先证明候选能被目标系统正确执行，Task6 再用固定 workload 判断是否值得采用。每个候选都经过同一组门槛：

| 门槛 | 通过条件 | 不通过时怎么处理 |
|:---|:---|:---|
| Artifact | 路径、版本、配置和完整性可复查 | 回到转换或产物整理 |
| Backend | loader 能识别格式，未出现未说明的 fallback | 记录兼容性失败并 tune / reject |
| Kernel | dtype、kernel 或 profiler 证据符合预期 | 不把文件变小写成速度收益 |
| Quality | 固定评测集和业务样例达到门槛 | 调整校准、粒度或回到 QLoRA |
| Performance | 显存、TTFT、TPOT、吞吐和 P95/P99 满足目标 | 继续调优 workload 或实现路径 |
| Repeatability | 相同配置重复运行结论方向一致 | 标记为证据不足，暂不 accept |

这组门槛把 `65 → 66 → 67` 的项目链路接到专题路线：65 提供训练适配产物，66 固定浮点参照，67 负责真实量化 artifact 与 backend 对照，最终由本节形成可解释的部署决策。
