...
Task0 的判断不能停在“对象分类”。还需要把对象、介入时机和证据类型连起来：
...
Part 00 的 Tensor、dtype、device 基础帮助学习者读懂表示变化；Part 01 的数据类型、GPU 执行路径和 KV Cache 页面帮助判断硬件与预算约束。它们提供判断所需的共同语言，真正的量化方法与部署验证仍在 Part 02 和本专题后续页面完成。
