...
**显存路线视角：** 计算图中的中间结果可能需要为 backward 保留，这解释了 activation 为什么会形成动态显存压力；本页只建立机制直觉，不测量真实 GPU 峰值。实际峰值和策略收益，留到 Part00 的显存账本以及 Part02 的 73 / 76 验证。这里负责回答“为什么反向传播可能需要保存中间结果”，不负责判断某个模型实际保存了多少显存。

**和 Part02 的关系：** 本页是通用入口：先建立 `requires_grad`、计算图、叶子张量和 `backward()` 的共同语言。Part02 的 18 节把这套语言用于激活函数和损失函数的局部梯度，17 节则进一步把它用于 Attention backward 和自定义 `autograd.Function`；两节都不替代真实 GPU benchmark。
