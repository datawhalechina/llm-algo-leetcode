...
**显存路线视角：** 训练循环把 forward、backward、optimizer step 和 validation 的生命周期串起来，可用来定位显存峰值可能出现的阶段，但本页不测真实峰值。训练性能与显存结论由 Part02 的 73 / 76 给出。训练 checkpoint 仍是恢复状态的保存机制，不等同于 activation checkpoint 的重算机制。
