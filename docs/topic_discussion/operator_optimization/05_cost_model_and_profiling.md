...
原 [Part 02 · 44 通用预留](../../02_PyTorch_Algorithms/44_Reserved_44.md) 的自动调优内容已归入本页：先用显存和延迟约束筛掉不可行配置，再对候选配置评分，最后保留候选、约束、评分和推荐理由。这里的重点是把 autotune 当作受约束的搜索与证据记录，而不是只报告一次最快运行。
