...
**要求**：请补全下方代码，手动将一个大规模的矩阵乘法拆分成两张“逻辑卡”上的 Column Parallel 与 Row Parallel 操作，并验证结果（拼接 / 求和后）与单卡全量计算一致。
...
def tensor_parallel_column_sim(X: torch.Tensor, A: torch.Tensor, num_gpus: int = 2):
    ...
    for i in range(num_gpus):
        ...
        # 可以使用 start_idx，end_idx 对 A 切片
        # 也可以不使用上述 idx，直接用 chunk 切片
        # torch.chunk(...)[i]
        # ==========================================
        # weight_chunk = ???
...
def tensor_parallel_row_sim(X: torch.Tensor, A: torch.Tensor, num_gpus: int = 2):
    ...
    将输入 X 沿输入特征维度 (dim=1) 切分，
    将权重 A 沿输入特征维度 (dim=0，即行方向) 切分，
    分布到不同的 GPU 上计算，最后将各卡的部分结果求和 (All-Reduce)。
    ...
    # 1. 模拟将输入与权重加载到不同 GPU 的显存中
    # ==========================================
    # TODO 1: 使用 torch.chunk 切分 X 和 A
    # X 沿输入特征维度 (dim=1) 切分，A 沿输入特征维度 (dim=0) 切分
    # ==========================================
    # x_chunks = ???
    # a_chunks = ???
    ...
    for i in range(num_gpus):
        # ==========================================
        # TODO 2: 每张卡使用本地的输入分片与权重分片进行矩阵乘法计算
        # ==========================================
        # local_out = ???
        # gpu_outputs.append(local_out)
        pass
        
    # 3. 模拟 All-Reduce 通信操作
    # ==========================================
    # TODO 3: 将各 GPU 的部分结果逐元素相加
    # ==========================================
    # Y_reduced = ???
    # return Y_reduced
    pass
...
def test_tensor_parallel():
    try:
        ...
        # 2. 模拟 2 张卡的 Column Parallel 与 Row Parallel
        Y_tp = tensor_parallel_column_sim(X, A, num_gpus=2)
        Y_tp_row = tensor_parallel_row_sim(X, A, num_gpus=2)
        ...
        diff = torch.max(torch.abs(Y_ref - Y_tp_row))
        print(f"最大误差: {diff.item():.6e}")
        assert diff < 1e-5, "TP 模拟结果与单卡全量计算不一致！"
        
        print("✅ Row Parallel (行切分) 矩阵计算与求和逻辑正确！")
        print("结合 Column Parallel，你已经掌握了 Megatron-LM 中 MLP 层的完整切分方案。")
...
def tensor_parallel_row_sim(X, A, num_gpus):
    ...
    a_chunks = torch.chunk(A, num_gpus, dim=0)
    
    # 2. 独立计算 (Local MatMul)
    y_chunks = []
    for i in range(num_gpus):
        y_local = x_chunks[i] @ a_chunks[i]
        y_chunks.append(y_local)
