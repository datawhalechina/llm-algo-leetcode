# 14 FlashAttention Sim

> 🚀 **云端运行环境**
> 
> 本章节的实战代码可以点击以下链接在免费 GPU 算力平台上直接运行：
> 
> [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/lynnyulinlin-debug/llm-algo-leetcode/blob/main/02_PyTorch_Algorithms/14_FlashAttention_Sim.ipynb)  
...
了解了基础的 Online Softmax 和分块机制后，我们来看看业界是如何一步步榨干 GPU（特别是 A100 到 H100）极限性能的。这是大厂高阶岗位的必考题。
...
> - **核心创新 1：WGMMA 异步计算**。H100 引入了 Warp Group (4 个 Warp 为一组)，FA3 使用了极具侵入性的低级汇编指令，允许 Tensor Core 在后台异步执行计算，而不阻塞寄存器。
> - **核心创新 2：TMA (Tensor Memory Accelerator)**。H100 专有的硬件级内存搬运器。FA3 让 TMA 自动从全局显存抓取数据到共享内存 (SRAM)，彻底解放了用于搬运数据的线程。
> - **核心创新 3：2-Stage to Ping-Pong Pipeline**。V1/V2 是两级流水线（Load -> Compute）。FA3 利用 TMA 实现了极致的软件流水线掩盖延迟，真正做到了“计算和访存时间的完美隐藏”。
