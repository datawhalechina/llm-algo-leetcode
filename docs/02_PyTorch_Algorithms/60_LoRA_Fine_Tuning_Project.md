...
本项目的 SFT 产物也是后续对齐的起点：保存底座模型版本、SFT checkpoint 或 LoRA adapter、tokenizer/chat template、数据切分和验证结果。若进入 DPO/GRPO，必须明确使用哪一个 checkpoint 初始化 policy/reference，并保持评测集与 SFT 阶段可追溯。
