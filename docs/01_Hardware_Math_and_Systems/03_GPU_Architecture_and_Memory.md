...
这是一节**硬件前置节**：它解释 GPU 层级、带宽和片上存储如何影响访存瓶颈，主要服务 `推理优化路线`，也为 `编译与图优化专题` 提供硬件背景。

**关键词：** `Tensor Core`, `SRAM`, `HBM`

## 证据边界与显存路线映射
本节是 **GPU optional**：CPU 模拟可以帮助理解 HBM、L2、SRAM、带宽和 arithmetic intensity 的相对关系；只有真实 GPU 才能验证具体硬件的带宽、kernel 选择、利用率、峰值显存和 OOM 边界。
